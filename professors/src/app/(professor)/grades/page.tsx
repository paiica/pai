...
function AttemptHistory({ lessonId, studentUserId, token }: { lessonId: string; studentUserId: string; token: string }) {
  const { data: attempts } = useSWR(
    [`/prof/lessons/${lessonId}/submissions/${studentUserId}/attempts`, token],
    ([url, t]) => fetcher(url, t)
  );
...
function LessonStatsHeader({ lessonId, token }: { lessonId: string; token: string }) {
  const { data: stats } = useSWR(
    [`/prof/lessons/${lessonId}/statistics`, token],
    ([url, t]) => fetcher(url, t)
  );
...
export default function ProfGradesPage() {
  ...
  async function handleExport() {
    ...
    setExporting(true);
    try {
      const res = await fetch(`${API_BASE}/prof/certifications/${activeCertId}/submissions/export`, {
  ...
  // one assignment at a time, not one flat list across the whole certification.
