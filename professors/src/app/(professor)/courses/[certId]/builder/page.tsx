...
export default function CourseBuilderPage() {
  ...
  const swrKey = token ? [`/prof/courses/${certId}`, token] as const : null;
  ...
  async function addModule() {
    ...
    await toast.promise(
      api.post<any>(`/prof/courses/${certId}/modules`, { title: moduleTitle }, token)
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            {cert.acronym} · {modules.length} modules · {modules.reduce((s: number, m: Module) => s + (m.lessons?.length ?? 0), 0)} lessons
          </p>
        </div>
