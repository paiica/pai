...
export default function ProfCertificationsPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Manage exams and grade students for your assigned certifications.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="card p-12 text-center text-slate-500">
          ...
          <p className="text-sm mt-1">Contact an administrator to be assigned as an examiner.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {certs.map((cert: any) => (
            <div key={cert.id} className="card p-5 flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 ml-4">
                ...
                <Link
                  href={`/grades?cert=${cert.id}`}
                  className="btn-outline text-xs px-3 py-2"
                >
                  <ChevronRight size={14} /> Submissions
                </Link>
              </div>
            </div>
          ))}
