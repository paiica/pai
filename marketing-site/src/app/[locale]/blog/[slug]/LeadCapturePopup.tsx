...
export default function LeadCapturePopup() {
  const t = useTranslations("BlogSubscribePopup");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await fetch(`${API}/leads`, {
        ...
        body: JSON.stringify({ email: email.trim(), source: "blog", page_url: window.location.href }),
      });
      localStorage.setItem(SEEN_KEY, "1");
      setSubmitted(true);
      ...
      <div className="relative w-full max-w-sm bg-white rounded-2xl shadow-2xl p-6">
        ...
        {submitted ? (
          <div className="text-center py-4">
            ...
            <p className="font-display font-black text-ink-900 text-lg">{t("subscribed")}</p>
            <p className="text-sm text-slate-500 mt-1">{t("newPostsInInbox")}</p>
          </div>
        ) : (
          <>
