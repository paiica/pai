...
async function getCmsPage(locale: string): Promise<CmsPage | null> {
  try {
    const res = await fetch(`${API}/pages/public/programs?lang=${locale}`, { cache: "no-store" });
...
type ProgramsT = (key: string, values?: Record<string, any>) => string;

async function getPrograms(locale: string): Promise<{ items: any[]; failed: boolean }> {
  try {
    const res = await fetch(`${API}/programs?lang=${locale}`, { cache: "no-store" });
...
function ProgramCard({ program, idx, t }: { program: any; idx: number; t: ProgramsT }) {
    ...
    <div className={cn(
      ...
      <div className="p-6 flex flex-col flex-1">
        <p className={cn("text-[12px] font-semibold mb-1.5", theme.dark ? "text-white" : "text-ink-900")}>
          {(LEVEL_LABEL_KEYS[program.level] ? t(LEVEL_LABEL_KEYS[program.level]) : program.level)}
          {program.duration_weeks ? ` · ${t("weeksCount", { count: program.duration_weeks })}` : ""}
          {program.course_count ? ` · ${t("courseCount", { count: program.course_count })}` : ""}
        </p>
...
export default async function ProgramsListPage() {
  ...
  const { items: programs, failed } = await getPrograms(locale);
  const cms = await getCmsPage(locale);
        ...
        {cms?.hero_enabled ? (
          ...
          <section className="pb-20 bg-hero-dark relative overflow-hidden" style={{ paddingTop: "calc(var(--header-height, 88px) + 48px)" }}>
            ...
            <div className="container-lg relative">
              ...
              <span className="badge-dark mb-5">{t("trainingProgramsBadge")}</span>
          ...
          <div className="container-lg">
            ...
            {!failed && programs.length === 0 && (
              <div className="text-center py-20">
                ...
                <p className="text-ink-500 text-sm">{t("noProgramsPublished")}</p>
              </div>
            )}
            {programs.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {programs.map((program: any, idx: number) => (
                  <ProgramCard key={program.id ?? program.slug} program={program} idx={idx} t={t} />
                ))}
              </div>
            )}
          </div>
