...
async function getCmsPage(locale: string): Promise<CmsPage | null> {
  try {
    const res = await fetch(`${API}/pages/public/privacy?lang=${locale}`, { cache: "no-store" });
...
const SECTIONS = [
  ...
  { title: "Data Storage & Security", content: "Your data is stored on encrypted servers in Canada and the United States, protected by TLS encryption in transit and encryption at rest. Access to student records is restricted to PAII staff who need it to do their job — such as reviewing an application or issuing a certificate — through role-based permissions, not open to the whole organization. Documents you upload (such as identification for exam proctoring) are kept in access-controlled cloud storage, separate from the rest of our systems. Payment data is handled entirely by Stripe, a PCI-compliant payment processor, and never touches or is stored on our own servers. We follow industry-standard security practices and review our access controls on an ongoing basis as the platform grows." },
