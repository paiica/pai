...
function buildEligibilityItems(opts: {
  ...
  if (opts.minYearsExperience != null) {
    items.push(t("yearsExperienceItem", { count: opts.minYearsExperience }));
  }
  if (opts.minTrainingHours != null) {
    items.push(t("trainingHoursItem", { count: opts.minTrainingHours }));
  }
  if (opts.industryFocus?.length) {
    items.push(t("industryFocusItem", { list: opts.industryFocus.join(", ") }));
  }
  if (opts.requiredEducation?.length) {
    items.push(t("relevantEducationItem", { list: opts.requiredEducation.join(", ") }));
  }
  if (opts.requiredDocuments?.length) {
    items.push(t("requiredDocumentsItem", { list: opts.requiredDocuments.join(", ") }));
  }
  return items;
}
...
export default function CertTabs({
  ...
  const t = useTranslations("CertTabs");
  ...
  const tabs = [
    { id: "right_for_you", label: t("tabRightForYou", { acronym }) },
    { id: "path",          label: t("tabPath") },
    { id: "prepare",       label: t("tabPrepare") },
      ...
      <div>
        ...
        >
          <div className="container-lg">
            <RightForYouTab
              ...
              acronym={acronym}
              applyUrl={applyUrl}
              headline={rfy.headline ?? t("defaultRightForYouHeadline", { acronym })}
              body={rfy.body ?? ""}
              stats={rfy.stats ?? []}
              requirements={targetAudience}
              eligibilityItems={buildEligibilityItems({ minYearsExperience, minTrainingHours, industryFocus, requiredEducation, requiredDocuments }, t)}
        ...
        >
          <div className="container-lg">
            <PathTab
              t={t}
              headline={path.headline ?? t("tabPath")}
              body={path.body ?? ""}
              steps={path.steps ?? []}
        ...
        >
          <div className="container-lg">
            <PrepareTab
              t={t}
              headline={prep.headline ?? t("tabPrepare")}
              body={prep.body ?? ""}
        ...
        >
          <div className="container-lg">
            <MaintenanceTab
              t={t}
              acronym={acronym}
              headline={mnt.headline ?? t("defaultMaintenanceHeadline", { acronym })}
              body={mnt.body ?? ""}
...
function RightForYouTab({
  t, acronym, applyUrl, headline, body, stats, requirements, eligibilityItems, notReadyText, notReadyHref,
}: {
  t: TFunc; acronym: string; applyUrl: string; headline: string; body: string;
    ...
    <TwoCol
      left={
        <>
          ...
          {requirements.length > 0 && (
            <div className="mb-8">
              <h3 className="text-base font-display font-bold text-ink-900 mb-4">{t("whoShouldPursue")}</h3>
              <BulletList items={requirements} />
            </div>
          )}
          {eligibilityItems.length > 0 && (
            <div className="mb-8">
              <h3 className="text-base font-display font-bold text-ink-900 mb-4">{t("eligibilityRequirements")}</h3>
      ...
      right={
        <div className="bg-sand-50 rounded-2xl border border-sand-200 p-8">
          {notReadyText ? (
            <>
              <h3 className="text-lg font-display font-bold text-ink-900 mb-3">{t("notReadyYet")}</h3>
            ...
            <>
              <h3 className="text-lg font-display font-bold text-ink-900 mb-3">{t("aboutTheAcronym", { acronym })}</h3>
              <p className="text-sm text-ink-900 leading-relaxed">
                {t("globallyRecognizedBody")}
              </p>
              <div className="mt-6">
                <a href={applyUrl} className="btn-outline !py-2.5 !px-6 !text-sm inline-flex">
                  {t("startYourApplication")} <ArrowRight size={14} />
                </a>
              </div>
            </>
          )}
...
function PathTab({
  t, headline, body, steps, learningOutcomes, notReadyText, notReadyHref,
}: {
  t: TFunc; headline: string; body: string; steps: Step[]; learningOutcomes: string[];
  ...
  if (displaySteps.length === 0) {
    return (
      <div className="max-w-2xl">
        <Heading>{headline}</Heading>
        <p className="text-slate-400 text-sm">{t("pathDetailsComingSoon")}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="bg-sand-50 border border-t-0 border-sand-200 rounded-b-2xl p-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div>
            {current.description ? (
              ...
              <p className="text-slate-400 text-sm italic">{t("stepContentComingSoon")}</p>
            )}
          </div>
          ...
          {notReadyText && (
            <div className="border-l border-sand-200 pl-10">
              <h3 className="text-base font-display font-bold text-ink-900 mb-3">{t("notReadyForCerts")}</h3>
        ...
        <div className="flex items-center justify-between mt-8 pt-6 border-t border-sand-200">
          ...
          >
            {t("viewFaqs")} <ChevronDown size={14} className="-rotate-90" />
          </button>
...
function PrepareTab({
  t, headline, body, resources, curriculum, totalLessons, totalHours,
}: {
  t: TFunc; headline: string; body: string; resources: Resource[];
      ...
      left={
        <>
          ...
          {resources.length > 0 ? (
            ...
            <p className="text-slate-400 text-sm">{t("studyResourcesComingSoon")}</p>
          )}
        </>
      }
      right={
        curriculum.length > 0 ? (
          <>
            <h3 className="text-base font-display font-bold text-ink-900 mb-1.5">{t("curriculumLabel")}</h3>
            <p className="text-xs text-slate-500 mb-5">
              {t("moduleCount", { count: curriculum.length })}
              {totalLessons > 0 && ` · ${t("lessonsCount", { count: totalLessons })}`}
              {totalHours > 0 && ` · ${t("hoursPlus", { count: totalHours })}`}
            </p>
          ...
          <div className="bg-white rounded-2xl border border-sand-200 p-8 text-center">
            ...
            <p className="text-slate-500 text-sm">{t("curriculumDetailsComingSoon")}</p>
          </div>
        )
      }
    />
...
function FaqsTab({ faqs, t }: { faqs: FaqItem[]; t: TFunc }) {
  const [open, setOpen] = useState<number | null>(null);

  if (!faqs.length) return (
    <div className="text-center py-16">
      <p className="text-slate-400 text-sm">{t("noFaqsYet")}</p>
    </div>
  );

  return (
    <div className="max-w-2xl">
      <Heading>{t("faqsHeading")}</Heading>
...
function MaintenanceTab({
  t, acronym, headline, body, renewalItems,
}: {
  t: TFunc; acronym: string; headline: string; body: string; renewalItems: string[];
}) {
  return (
    <div className="max-w-2xl">
      <Heading>{headline || t("defaultMaintenanceHeadline", { acronym })}</Heading>
      {body && <Body>{body}</Body>}
      {renewalItems.length > 0 ? (
        <BulletList items={renewalItems} />
      ) : (
        <p className="text-slate-400 text-sm">{t("maintenanceDetailsComingSoon")}</p>
      )}
    </div>
  );
}
