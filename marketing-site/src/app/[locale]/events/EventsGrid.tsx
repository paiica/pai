...
const NATURE_LABEL_KEYS: Record<string, string> = {
  training: "natureTraining", seminar: "natureSeminar", workshop: "natureWorkshop",
  conference: "natureConference", meetup: "natureMeetup", webinar: "natureWebinar", other: "natureOther",
};

const PLATFORM_LABEL_KEYS: Record<string, string> = {
  zoom: "platformZoom", teams: "platformTeams", google_meet: "platformGoogleMeet", other: "platformOther",
};

const MODE_LABEL_KEYS: Record<string, string> = {
  in_person: "modeInPerson", hybrid: "modeHybrid", online: "modeOnline",
};
    ...
    <div>
      {(availableNatures.length > 1 || availableModes.length > 1) && (
        <div className="flex flex-wrap items-center gap-2 mb-10">
          <FilterChip active={nature === "all"} onClick={() => setNature("all")}>{t("filterAll")}</FilterChip>
          {availableNatures.map((n) => (
            <FilterChip key={n} active={nature === n} onClick={() => setNature(n)}>
              {t(NATURE_LABEL_KEYS[n] ?? "natureOther")}
            </FilterChip>
          ))}
          ...
          {availableModes.map((m) => (
            <FilterChip key={m} active={mode === m} onClick={() => setMode(mode === m ? "all" : m)}>
              {t(MODE_LABEL_KEYS[m] ?? "modeOnline")}
            </FilterChip>
          ))}
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-sand-500 text-sm">{t("noEventsMatchFilters")}</div>
      ) : (
                  ...
                  <div className="mt-auto pt-5 border-t border-sand-200 space-y-3">
                      ...
                      <span>
                        ...
                        {event.event_type === "hybrid" && (
                          <>
                            {t("modeHybrid")}
                            {event.city && <> · <span className="font-black text-ink-900">{event.city}</span></>}
                            {" "}· {t("viaPlatform", { platform: t(PLATFORM_LABEL_KEYS[event.meeting_platform ?? ""] ?? "modeOnline") })}
                          </>
                        )}
                        {event.event_type === "online" && (
                          <>{t("modeOnline")}{event.meeting_platform && <> · {t("viaPlatform", { platform: t(PLATFORM_LABEL_KEYS[event.meeting_platform]) })}</>}</>
                        )}
                      </span>
                    </div>
                    <div className="flex justify-end">
                      {price > 0 ? (
                        ...
                        <span className="text-base font-bold text-teal-600 uppercase tracking-wide">{t("free")}</span>
                      )}
                    </div>
                  </div>
                </div>
