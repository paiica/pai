...
async function getRelatedCourses(slugs: string[], locale: string): Promise<Course[]> {
...
export default async function CourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const tabs = [
    learningOutcomes.length > 0 && { id: "course-section-learn", label: t("whatYoullLearn") },
    howItWorksSteps.length > 0 && { id: "course-section-how", label: t("howItWorks") },
    modules.length > 0 && { id: "course-section-curriculum", label: t("curriculumLabel") },
    documents.length > 0 && { id: "course-section-materials", label: documents.length > 1 ? t("materialsCount", { count: documents.length }) : t("materials") },
    (content.training_exam_prep_body || prepItems.length > 0) && { id: "course-section-included", label: t("whatsIncluded") },
    instructors.length > 0 && { id: "course-section-instructors", label: t("instructorsLabel") },
  ].filter(Boolean) as { id: string; label: string }[];

  const relatedSlugs = safeArray<string>(content.related_course_slugs);
  const relatedCourses = await getRelatedCourses(relatedSlugs, locale);
      ...
      <main>
        ...
        >
          <div className="container-lg relative">
            ...
            <div className="flex items-center gap-2 text-navy-700/60 text-xs font-semibold mb-5">
              <Link href="/" className="hover:text-navy-900 transition-colors">{tc("home")}</Link>
              <ChevronRight size={12} />
              <Link href="/courses" className="hover:text-navy-900 transition-colors">{t("coursesBreadcrumb")}</Link>
              <ChevronRight size={12} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
              ...
              <div className="lg:col-span-2">
                <div className="flex items-center gap-2 mb-4 flex-wrap">
                  ...
                  {course.level && (
                    <span className="text-xs font-semibold px-3 py-1 rounded-full border border-navy-900/30 bg-white/40 text-navy-800 backdrop-blur-sm">
                      {LEVEL_LABEL_KEYS[course.level] ? t(LEVEL_LABEL_KEYS[course.level]) : course.level}
                    </span>
                  )}
                  {course.cert_acronym && (
                    <span className="text-xs font-semibold px-3 py-1 rounded-full border border-navy-900/30 bg-white/40 text-navy-800 backdrop-blur-sm">
                ...
                <div className="flex flex-wrap gap-5 text-sm text-navy-700 mb-6">
                  {course.duration_hours > 0 && (
                    <span className="flex items-center gap-1.5"><Clock size={14} />{t("hoursOfContent", { count: course.duration_hours })}</span>
                  )}
                  {totalLessons > 0 && (
                    <span className="flex items-center gap-1.5"><BookOpen size={14} />{t("lessonsCount", { count: totalLessons })}</span>
                  )}
                  {modules.length > 0 && (
                    <span className="flex items-center gap-1.5"><BarChart2 size={14} />{t("moduleCount", { count: modules.length })}</span>
                  )}
                </div>
              ...
              <div className="bg-white rounded-2xl p-7 shadow-xl border border-sand-200">
                {isLaunchPricing && (
                  <div className="flex items-center gap-2 mb-2.5">
                    ...
                    <span className="text-[11px] text-ink-900/60">{t("limitedTime")}</span>
                  </div>
                )}
                ...
                <div className="text-ink-900 text-sm mb-5">
                  {price > 0 ? t("oneTimeFeeLifetime") : t("noCreditCardRequired")}
                </div>

                <div className="space-y-2.5 mb-6">
                  {[
                    course.duration_hours > 0 ? t("hoursOfContentPlus", { count: course.duration_hours }) : null,
                    totalLessons > 0
                      ? (modules.length > 0 ? t("lessonsInModules", { lessons: totalLessons, modules: modules.length }) : t("lessonsCount", { count: totalLessons }))
                      : null,
                    ...enrollmentIncludes,
          ...
          <section id="course-section-learn" className="section-padding bg-white border-b border-sand-200">
            <div className="container-lg">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div>
                  <h2 className="text-3xl font-display font-black text-ink-900">{t("whatYoullLearn")}</h2>
                </div>
          ...
          <section id="course-section-how" className="section-padding bg-sand-50 border-b border-sand-200">
            <div className="container-lg">
              <h2 className="text-3xl font-display font-black text-ink-900 mb-2">
                {content.how_it_works_headline || t("howItWorks")}
              </h2>
              <p className="text-slate-500 text-sm mb-10">{t("stepByStepPath")}</p>
        ...
        {modules.length > 0 && (
          <section id="course-section-curriculum" className="section-padding bg-white border-b border-sand-200">
            <div className="container-lg">
              <h2 className="text-3xl font-display font-black text-ink-900 mb-2">{t("courseCurriculum")}</h2>
              <p className="text-slate-500 text-sm mb-8">{t("moduleCount", { count: modules.length })} · {t("lessonsCount", { count: totalLessons })}</p>
        ...
        {documents.length > 0 && (
          <section id="course-section-materials" className="section-padding bg-sand-50 border-b border-sand-200">
            <div className="container-lg">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div>
                  <h2 className="text-3xl font-display font-black text-ink-900">{t("courseMaterials")}</h2>
                  <p className="text-slate-500 text-sm mt-2">{t("previewSyllabus")}</p>
                </div>
                  ...
                  {documents.map((doc) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 truncate">{doc.file_name || t("download")}</p>
                      </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                <div>
                  <h2 className="text-3xl font-display font-black text-ink-900">
                    {content.training_exam_prep_headline || t("whatsIncluded")}
                  </h2>
                </div>
        ...
        {relatedCourses.length > 0 && (
          <section className="section-padding bg-white border-t border-sand-200">
            <div className="container-lg">
              <h2 className="text-2xl font-display font-bold text-ink-900 mb-2">{t("youMightAlsoLike")}</h2>
              <p className="text-slate-500 text-sm mb-8">{t("moreCoursesToAdvance")}</p>
