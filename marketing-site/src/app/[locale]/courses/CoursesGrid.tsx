      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-10">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {availableLevels.length > 1 && (
            <>
              <FilterChip active={level === "all"} onClick={() => setLevel("all")}>{t("allLevels")}</FilterChip>
              {availableLevels.map((l) => (
                <FilterChip key={l} active={level === l} onClick={() => setLevel(l)}>
                  {LEVEL_LABEL_KEYS[l] ? t(LEVEL_LABEL_KEYS[l]) : l}
                </FilterChip>
              ))}
              <div className="w-px h-6 bg-sand-300 mx-1" />
            </>
          )}
          <FilterChip active={price === "all"} onClick={() => setPrice("all")}>{t("allPrices")}</FilterChip>
          <FilterChip active={price === "free"} onClick={() => setPrice(price === "free" ? "all" : "free")}>{t("free")}</FilterChip>
          <FilterChip active={price === "paid"} onClick={() => setPrice(price === "paid" ? "all" : "paid")}>{t("paid")}</FilterChip>
        </div>
      </div>

      {filtered.length === 0 ? (
        <div className="py-20 text-center text-sand-500 text-sm">{t("noMatches")}</div>
      ) : (
