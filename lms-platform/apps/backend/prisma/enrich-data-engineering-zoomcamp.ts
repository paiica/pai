/**
 * Enriches Data Engineering Zoomcamp with blocks-based content; only
 * 06-batch gets a lab (see seed-data-engineering-zoomcamp.ts).
 * Run with: npx ts-node prisma/enrich-data-engineering-zoomcamp.ts
 * Safe to re-run.
 */
...
import { MODULES } from "./seed-data-engineering-zoomcamp";

const prisma = new PrismaClient();
const { fetchText, resolveRelativeUrls } = makeRepoHelpers("paiica", "data-engineering-zoomcamp");
...
async function main() {
  console.log("🌱  Enriching Data Engineering Zoomcamp…\n");
  const course = await prisma.course.findUnique({ where: { slug: "data-engineering-zoomcamp" } });
  if (!course) throw new Error("Run seed-data-engineering-zoomcamp.ts first");
  ...
  for (const modDef of MODULES) {
    ...
    for (const { folder, notebook } of modDef.lessons) {
      let readme = await fetchText(`${folder}/README.md`);
      if (!readme) { console.warn(`⚠ No README: ${folder}`); continue; }
      const lessonTitle = cleanTitle(resolveLessonTitle(readme, folder));
      // 07-streaming's own README is just a link list — the real content is
      // in its workshop subfolder (confirmed: 48KB vs ~1KB).
      if (folder === "07-streaming") {
        const workshop = await fetchText(`${folder}/workshop/README.md`);
        if (workshop) readme = `${readme}\n\n${workshop}`;
      }
