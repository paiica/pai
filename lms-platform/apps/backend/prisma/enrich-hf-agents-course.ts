/**
 * Enriches Hugging Face Agents Course: fetches each lesson's .mdx page
 * (plain Markdown with occasional raw HTML) and saves it as blocks.
 *
 * Run with: npx ts-node prisma/enrich-hf-agents-course.ts
 * Safe to re-run.
 */
...
import { MODULES } from "./seed-hf-agents-course";

const prisma = new PrismaClient();
const { fetchText, resolveRelativeUrls } = makeRepoHelpers("paiica", "agents-course", "huggingface");

async function main() {
  console.log("🌱  Enriching Hugging Face Agents Course…\n");
  const course = await prisma.course.findUnique({ where: { slug: "hf-agents-course" } });
  if (!course) throw new Error("Run seed-hf-agents-course.ts first");
  ...
  for (const modDef of MODULES) {
    ...
    for (const lessonDef of modDef.lessons) {
      ...
      const md = await fetchText(`units/en/${lessonDef.path}.mdx`);
