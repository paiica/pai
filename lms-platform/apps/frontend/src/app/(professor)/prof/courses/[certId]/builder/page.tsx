...
function ModuleSection({ module, certId, token, onRefresh, index, total }: {
  ...
  async function moveModule(dir: "up" | "down") {
    // Get sibling IDs from parent — handled by bulk reorder
    toast("Drag-and-drop reorder coming soon — use up/down arrows to shift.");
  }
    ...
    <div className="card border border-slate-200 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-navy-50 border-b border-slate-100">
        ...
        <div className="flex items-center gap-1">
          {index > 0 && (
            <button onClick={() => moveModule("up")} className="p-1 rounded hover:bg-slate-200 text-slate-400">
              <ArrowUp size={13} />
            </button>
          )}
          {index < total - 1 && (
            <button onClick={() => moveModule("down")} className="p-1 rounded hover:bg-slate-200 text-slate-400">
              <ArrowDown size={13} />
            </button>
          )}
