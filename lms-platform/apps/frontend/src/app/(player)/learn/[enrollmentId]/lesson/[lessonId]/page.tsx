...
// Mirrors the same client-side computation used in the Certification/Course
// Builder's AssignmentEditor status strip, so what the student sees always
// matches what the instructor configured.
    ...
    <div className="space-y-5">
      ...
      {isGraded ? (
        ...
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700 flex items-center gap-2">
          <AlertCircle size={14} /> Maximum submissions reached ({attemptsUsed}/{maxAttempts}) — awaiting review by your professor
        </div>
      ) : submission ? (
        <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700 flex items-center gap-2">
          <CheckCircle size={14} /> Submitted{submission.is_late ? " (late)" : ""} — awaiting review by your professor
              ...
              <div className="space-y-2">
                ...
                {pendingFiles.length < maxFiles && (
                  <label className={cn(
                    ...
                    <span className="text-sm text-slate-500">
                      {uploading ? "Uploading…" : `Click to upload ${acceptedTypes.length ? acceptedTypes.join(", ") : "a file"}`}
                    </span>
                    <input
                      type="file"
                      multiple={maxFiles > 1}
...
export default function LessonPage() {
  ...
  const { lesson, progress, submission, submission_attempts: submissionAttempts } = data;
    ...
    <div className="px-6 py-8">
      ...
      <div className={cn("mb-8", !isHtmlLesson && "max-w-3xl mx-auto")}>
        ...
        {lesson.type === "assignment" && (
          <AssignmentLesson lesson={lesson} enrollmentId={enrollmentId} token={token} submission={submission} submissionAttempts={submissionAttempts} onComplete={() => mutate()} />
        )}
      </div>
