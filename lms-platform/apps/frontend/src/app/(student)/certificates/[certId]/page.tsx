...
// A template's <link rel="stylesheet"> to Google Fonts needs crossorigin
// set, or html-to-image's PNG export throws a SecurityError trying to read
// .cssRules off that cross-origin sheet to collect @font-face rules — a
// hand-authored template (admin-pasted HTML, not generated by this app) can
// easily omit it, so it's patched in here rather than relying on every
// template author remembering to add it themselves.
...
function CertificateSection({ issuedCert }: { issuedCert: any }) {
  ...
  function handleDownloadPdf() {
    ...
    // the Google Fonts <link>) have been parsed and applied — document.fonts
    // .ready + requestAnimationFrame turned out unreliable on a document
    // created via document.write() and produced a blank print preview
    // (printing before layout had committed at all, not just before fonts
    // settled). The trailing delay gives font FILES a little extra margin —
    // the signature is now a self-contained inline base64 @font-face so it
    // no longer depends on this at all, but Archivo/EB Garamond still fetch
    // over the network.
  ...
  async function handleDownloadPng() {
    ...
    try {
      ...
      // style: the PDF's @media print rule explicitly zeroes .sheet's box-
      // shadow (a page floating above another page doesn't make sense once
      // printed) — toPng captures under normal screen styles, which never
      // triggers that rule, so without this override the PNG would keep the
      // drop shadow the PDF doesn't have. Overrides the clone made for
      // capture only, not the live on-screen node.
