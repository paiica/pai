...
const LEVEL_GROUPS = [
  { key: "pre_certificate", labelKey: "groupPreCertificateLabel", descriptionKey: "groupPreCertificateDesc" },
  { key: "foundation",      labelKey: "levelFoundation",          descriptionKey: "groupFoundationDesc" },
  { key: "advanced",        labelKey: "levelAdvanced",            descriptionKey: "groupAdvancedDesc" },
  { key: "specialist",      labelKey: "levelSpecialist",          descriptionKey: "groupSpecialistDesc" },
  { key: "executive",       labelKey: "levelExecutive",           descriptionKey: "groupExecutiveDesc" },
  { key: "other",           labelKey: "levelOther",               descriptionKey: "groupOtherDesc" },
];
    ...
    <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden flex flex-col shadow-sm hover:shadow-md transition-all duration-200">
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          {cert.level && (
            <span className={cn("text-[10px] font-semibold px-1.5 py-0.5 rounded-md", LEVEL_COLOR[cert.level] ?? "bg-slate-100 text-slate-600")}>
              {LEVEL_LABEL_KEY[cert.level] ? t(LEVEL_LABEL_KEY[cert.level]) : cert.level}
            </span>
          )}
          {cert.duration_weeks && (
        ...
        <div className="flex items-center justify-between pt-3 border-t border-slate-100">
          <span className="font-black text-navy-900 text-sm">{price === 0 ? t("free") : `$${price.toFixed(2)}`}</span>
          {enrolled ? (
          ...
          >
            ...
            <div className="flex items-center gap-3 flex-shrink-0 ml-4">
              <span className="text-[11px] font-medium text-slate-400">
                {group.certs.length} {group.certs.length !== 1 ? t("certPlural") : t("certSingular")}
              </span>
              <ChevronDown
                size={15}
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 space-y-12">
        ...
        <section>
          <div className="flex items-start justify-between mb-6">
            <div>
              <h1 className="text-3xl font-display font-black text-navy-900 tracking-tight">{t("myCertificatesHeading")}</h1>
              <p className="text-sm text-slate-400 mt-1">{t("earnedSubheading")}</p>
            </div>
          </div>
          ...
          {certificates.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white py-16 text-center">
              ...
              <p className="font-display font-bold text-navy-900 text-base mb-1">{t("noCertsHeading")}</p>
                      ...
                      <div className="flex items-start justify-between gap-3 mb-1">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate-400 mt-0.5">
                            {cert.certificate_number} · {t("issued")} {formatDate(cert.issued_at)}
                          </p>
                        </div>
                        <span className={cn(
                          ...
                          {cert.status === "active" ? t("statusActive") : cert.status === "expired" ? t("statusExpired") : cert.status === "lapsed" ? t("statusLapsed") : t("statusRevoked")}
                        </span>
                      </div>
                        ...
                        <div className="flex items-center gap-5 mt-3 pt-3 border-t border-slate-100">
                          ...
                          {(() => {
                            ...
                            if (cert.status === "expired") {
                              return <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-100">{t("renewalAvailable")}</span>;
                            }
                            if (daysLeft <= 90) {
                              return <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-100">{t("expiresIn", { days: Math.max(daysLeft, 0) })}</span>;
                            }
                            return null;
                          })()}
                          <Link
        ...
        <section>
          <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
            <div>
              <h2 className="text-2xl font-display font-black text-navy-900 tracking-tight">{t("availableCertsHeading")}</h2>
              <p className="text-sm text-slate-400 mt-1">{t("availableCertsSubheading")}</p>
            </div>
            {items.length > 0 && (
              <Link
                href="/cart"
          ...
          {catalog.length === 0 ? (
            <div className="py-14 text-center border border-dashed border-slate-200 rounded-2xl bg-white">
              <p className="text-slate-400 text-sm font-semibold">{t("noCatalogCerts")}</p>
            </div>
          ) : (
